...
from hmfast.utils import Const, lambertw
...
class _NFWDensityProfile(DensityProfile):
    ...
    def real(self, halo_model, r, m, z):
        ...
        r_delta = (
            jnp.reshape(
                halo_model.mass_def.r_delta(halo_model.cosmology, m, z),
                (len(m), len(z)),
            )
            * cparams["h"]
        )
        ...
        # NaN-safe: a bare `x_s <= c_delta_b` would silently zero out NaN instead of propagating it.
        rho_gas = jnp.where(
            jnp.isnan(x_s) | jnp.isnan(c_delta_b) | (x_s <= c_delta_b), rho_gas, 0.0
        )
...
class _BCMDensityProfile(DensityProfile):
    ...
    def real(self, halo_model, r, m, z):
        ...
        r_vir = jnp.reshape(
            MassDefinition("vir", "critical").r_delta(halo_model.cosmology, m, z),
            (len(m), len(z)),
        )
        x_vir = rb / ((1.0 + zb) * r_vir[None, :, :])

        # Redshift Dependent Mc (Matching your C logic)
        mc_z_log = self.log10Mc * (1.0 + zb) ** self.nu_log10Mc
        mc = 10.0**mc_z_log
        ...
        beta_m = 3.0 * m_ratio_mu / (1.0 + m_ratio_mu)

        # Denominator 1: Large scale bound gas
        denom1 = (1.0 + 10.0 * x_vir) ** beta_m

        # Denominator 2: Ejected gas / transition,
        scaled_r = x_vir / self.theta_ej
        denom2 = (1.0 + (scaled_r) ** self.gamma) ** (
            (self.delta - beta_m) / self.gamma
        )
